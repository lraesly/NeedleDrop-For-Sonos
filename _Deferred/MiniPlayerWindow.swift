...
final class MiniPlayerWindow {
    ...
    func show(appState: AppState) {
        ...
        // Active when the mouse is inside OR the panel is the key window.
        ...
        resignKeyObserver = NotificationCenter.default.addObserver(
            ...
            // Only go transparent if the cursor is also outside the window
    ...
    func updateTitle(for track: TrackInfo?) {
        ...
        if let track, track.isTVAudio {
            panel.title = "TV Audio — \(track.zone)"
        } else if let track {
